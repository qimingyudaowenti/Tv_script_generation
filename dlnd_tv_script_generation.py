...
def get_batches(int_text, batch_size, seq_length):
    ...
    # remove last few characters to make only full batches
    ...
    # reshaping
    x_bat = np.split(x.reshape(batch_size, -1), n_batches, 1)
    y_bat = np.split(y.reshape(batch_size, -1), n_batches, 1)

    batches = np.array(list(zip(x_bat, y_bat)),dtype=np.int32)
#    print(batches)
    return batches
